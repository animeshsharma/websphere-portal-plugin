...
import maven.websphere.plugin.cli.ErrorConsumer;
import maven.websphere.plugin.cli.InfoConsumer;
...
import maven.websphere.plugin.utils.FtpWrapper;
import maven.websphere.plugin.utils.ResourceUtils;
...
public abstract class AbstractWebsphereMojo extends AbstractMojo {
	...
	/**
	 * WebSphere Portal Server version, options of 6.1 and 8.0
	 * 
	 * 
	 * @since 1.0.3
	 */
	@Parameter(property = "wpsVersion", defaultValue = "8.0")
	...
	private void extractAssets() throws MojoExecutionException {
		...
		if (!assetsDir.exists()) {
			try {
				...
				PlexusResource asset = resourceManager
						.getResource(getWpsVersion() + SLASH + getMode());
